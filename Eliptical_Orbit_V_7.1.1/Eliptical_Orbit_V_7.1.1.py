...
if e<1:
    ...
    Element_shapes = [[314.854,0.96,0.96,2.135,"Rect Prism"],
    ...
    print("\n--------Orbital torques -------\n")
    print(f"{'Type':<35}{'Maximum':<10}{'Minimum':<10}{'Unity':<}\n")
    print(f"{'Solar torques:':<35}{np.max(ST_abs_list):<10.5f}{np.min(ST_abs_list):<10.5f}{'[N.m]':<}")
    print(f"{'Gravity gradient torques:':<35}{np.max(GT_abs_list):<10.5f}{np.min(GT_abs_list):<10.5f}{'[N.m]':<}")
    print(f"{'Nadir pointing torques:':<35}{np.max(NT_abs_list):<10.5f}{np.min(NT_abs_list):<10.5f}{'[N.m]':<}")
